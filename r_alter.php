...
$pdo= new PDO("mysql:host=localhost;dbname=db_student","root","root"); 
		...
		$id = @$_POST['id'];
		$res = $pdo -> query("select * from tb_stu where id = '$id'"); 
		$row = $res -> fetch();
	// session_start();
	if (isset($_POST['subm'])) {
		...
		// $session = @$_SESSION['id'];
		// $sql = ;
		$pdo->exec("update tb_stu set stu_num='$stu_num',name='$name',class='$class',profession='$pro' where id = '$id'");
		// $update->execute();
		echo "<script>alert('修改成功!');location = 'admin_alt_stu.php';</script>";
	// }else {
        ...
        <section class="vbox">
          <section class="scrollable padder">
            <form method="post" action="r_alter.php">
				...
				<input type="text" name="pro" value="<?php echo $row['profession']; ?>">
